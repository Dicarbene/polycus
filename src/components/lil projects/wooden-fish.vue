...
onMounted(() => {
  ...
  woodenFish.value?.addEventListener('click', (e) => {
    const arr = [t('projects.wooden-fish.sounds.dong'), t('projects.wooden-fish.sounds.doo'), t('projects.wooden-fish.sounds.der')]
    const elem = document.createElement('b')
    elem.style.zIndex = '9999'
    elem.style.position = 'absolute'
    elem.style.userSelect = 'none'
    elem.style.left = `${e.clientX - 20 + Math.random() * 10}px`
    elem.style.top = `${e.clientY + Math.random() * 5}px`
    cnt = (cnt + 1) % 3
...
<template>
  <div w-80>
    <p>{{ t("projects.wooden-fish.name") }}</p>
    <div class="h-auto flex flex-col justify-center">
      <p>
        {{ t('projects.wooden-fish.benefaction') }}: {{ count }}/10
      </p>
      <div
        ref="woodenFish"
        class="my-2 m-5 border-rd-3 text-center bt-4 hover:bg-gray-200 hover:dark:bg-gray-600 transition-colors"
        relative
        @click="addGd"
      >
        <img
        ...
        >
        <img
          v-else :style="brightCursorStyle" w-80 h-60 opacity-80 src="/in-projects/wooden-fish/bowl.svg"
          @click="wave"
        >
        <Transition name="bounce">
          <img
            v-if="show && isDark" :style="darkCursorStyle" w-80 h-60 opacity-80 absolute top--1
            src="/in-projects/wooden-fish/dark-bowl.svg"
          >
        </Transition>
        <Transition name="bounce">
          <img
            v-if="show && !isDark" :style="brightCursorStyle" w-80 h-60 opacity-80 absolute top--1
